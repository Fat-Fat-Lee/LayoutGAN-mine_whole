...
# CUDA_VISIBLE_DEVICES=4,5,6,7 python -m torch.distributed.launch --nproc_per_node=2  train_gpus.py --dataset ricoTest --batch_size 64 --iteration 200000 --latent_size 8 --lr 5e-06 --G_d_model 512 --G_nhead 4 --G_num_layers 8 --D_d_model 512 --D_nhead 4 --D_num_layers 8
...
def main():
    ...
    if args.dataset == 'ricoTest':
        ...
        num_ricoLabel = -1
    # -------------------------------------------#
    tmpl = './output/ricoTest/LayoutGAN++/20230221184830307270/model_best.pth.tar'
    # #
    # # # # --------------by ljw 20221103------------------#
    ...
    # setup optimizer
    optimizerD = optim.Adam(netD.parameters(), lr=args.lr*0.1)
    optimizerG = optim.Adam(netG.parameters(), lr=args.lr*0.1)
    ...
    for epoch in range(max_epoch):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            D_real, logit_cls, bbox_recon = netD(bbox_real, label, padding_mask, reconst=True,
                                                 ricoLabel=ricoLabel)  # by ljw 20221212
            ...
            if iteration % 5000 == 0:
                ...
                with torch.no_grad():
                    ...
                    bbox_fake = netG(fixed_z, fixed_label, ~fixed_mask, fixed_ricoLabel)  # 加上ricoLabel
        ...
        # torch.cuda.empty_cache()  # by ljw 20220919
        # torch.cuda.empty_cache()  # by ljw 20220919
        print("epoch/max_epoch", epoch, "/", max_epoch, "  ", "is_best", is_best, "best_iou", best_iou, "max_iou_val",
              max_iou_val)
