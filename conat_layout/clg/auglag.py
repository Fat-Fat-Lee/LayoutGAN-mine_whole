...
class AugLagMethod():
    def __init__(self, netG, netD, inner_optimizer, constraints,
                 ...
                 clamp_f=True, raise_error_if_failed=False,ifRicoTest=False):#by ljw 20221110
        self.netG = netG
        self.netD = netD
        ...
        self.ifRicoTest=ifRicoTest#by ljw 20221110
    ...
    def f(self, bbox, label, padding_mask,ricoLabel=-1):#by ljw 20230104
        if(self.ifRicoTest is True):
            f = -torch.sigmoid(self.netD(bbox, label, padding_mask,ricoLabel=ricoLabel))
        else:
    ...
    def build_Adam_objective(self, l, m, data, label, padding_mask, mask_c,ricoLabel=-1):#by ljw 20230221
        def objective(z):
            # --------------------by ljw 20230221-----------------------#
            if self.ifRicoTest is True:
                bbox = self.netG(z, label, padding_mask,ricoLabel=ricoLabel)
                f = self.f(bbox, label, padding_mask,ricoLabel=ricoLabel)
            else:
    ...
    def build_CMAES_objective(self, l, m, data, label, padding_mask, mask_c,ricoLabel=-1):#by ljw 20230104
        def objective(x):
            B, P, ND = x.size()
            N = label.size(1)
            D = int(ND / N)
            ...
            #--------------------by ljw 20230104-----------------------#
            if self.ifRicoTest is True:
                _ricoLabel = ricoLabel.unsqueeze(1)
                _ricoLabel = _ricoLabel.expand(-1, P, -1).reshape(-1, N)
                bbox = self.netG(z, _label, _padding_mask,ricoLabel=_ricoLabel)
                f = self.f(bbox, _label, _padding_mask,ricoLabel=_ricoLabel).view(B, P)
            else:
            ...
            if self.clamp_f:
                f = torch.relu(f - self._f0[:f.size(0)])
            h = self.h(bbox.view(B, P, N, D), data, mask_c)#by ljw 20230221后续修改是否重新训练
    ...
    def generator(self, z, data):
        ...
        if self.ifRicoTest is True:
            ...
            bbox =self.netG(z, label, padding_mask, ricoLabel)
            if self.clamp_f:
                self._f0 = self.f(bbox, label, padding_mask,ricoLabel=ricoLabel)
        ...
        for _ in range(self.iteration):
            ...
            # ------------by ljw 20221110-------------#
            if self.ifRicoTest is True:
                objective = build(l, m, data, label,padding_mask, mask_c,ricoLabel=ricoLabel)#by ljw 20230104
            else:
                objective = build(l, m, data, label, padding_mask, mask_c)  # by ljw 20230104
            ...
            #---------by ljw 20221213--------#
            if self.ifRicoTest is True:
                bbox = self.netG(z, label, ~mask,ricoLabel)
            else:
