...
class LayoutNet(nn.Module):
    def __init__(self, num_label,num_ricoLabel=-1):
        super().__init__()
        ...
        num_layers = 4
        max_bbox = 50
        self.num_ricoLabel=num_ricoLabel#by ljw 20221103
        ...
        #     # encoder
        #     self.emb_label_new = nn.Embedding(num_label, d_model//2)
        #
        # else:
            # encoder
    ...
    def extract_features(self, bbox, label, padding_mask,ricoLabel=-1):
        b = self.fc_bbox(bbox)

        # -------------by ljw 20221103--------#
        # to do连接新特征
        # if self.num_ricoLabel!=-1:
        #     l = self.emb_label_new(label)  # by ljw 20221104
        #     r = self.emb_ricoLabel(ricoLabel)
        #     x = self.enc_fc_in(torch.cat([b, l,r], dim=-1))
        #
        # else:
        l = self.emb_label(label)  # by ljw 20221104
    ...
    def forward(self, bbox, label, padding_mask,ricoLabel=-1):#by ljw 20221103
        B, N, _ = bbox.size()

        #-----------by ljw 20221103--------------------#
        if self.num_ricoLabel!=-1:
            x = self.extract_features(bbox, label, padding_mask,ricoLabel)
        else:
