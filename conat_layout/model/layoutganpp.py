...
class Generator(nn.Module):
    def __init__(self, dim_latent, num_label,
                 d_model=512, nhead=8, num_layers=4,num_ricoLabel=-1):
        super().__init__()

        self.fc_z = nn.Linear(dim_latent, d_model // 2)
        self.num_ricoLabel=num_ricoLabel#by ljw 20221031
        # --------------------by ljw 20221031--------------------------#
        # to do新特征处理定义
        if num_ricoLabel!=-1:
            self.emb_ricoLabel=nn.Embedding(num_ricoLabel, d_model // 4)
            self.emb_label_new = nn.Embedding(num_label, d_model // 4)  # 改为//4 by ljw
        else:
    ...
    def forward(self, z, label, padding_mask,ricoLabel=-1):#by ljw 20221103

        z = self.fc_z(z)

        #-------------by ljw 20221031--------#
        #to do连接新特征
        if self.num_ricoLabel!=-1:
            l = self.emb_label_new(label)  # by ljw
            r=self.emb_ricoLabel(ricoLabel)
            x = torch.cat([z, l, r], dim=-1)

        else:
            l = self.emb_label(label)  # by ljw
...
class Discriminator(nn.Module):
    def __init__(self, num_label, d_model=512,
                 nhead=8, num_layers=4, max_bbox=50,num_ricoLabel=-1):
        super().__init__()
        self.num_ricoLabel = num_ricoLabel  # by ljw 20221212
        # encoder
        self.emb_label = nn.Embedding(num_label, d_model)
        # --------------------by ljw 20221212--------------------------#
        # to do新特征处理定义
        if num_ricoLabel != -1:
            self.emb_ricoLabel = nn.Embedding(num_ricoLabel, d_model // 2)
            self.emb_label_new = nn.Embedding(num_label, d_model // 2)  # 改为//2 by ljw
        else:
            self.emb_label = nn.Embedding(num_label, d_model)
        #-- -------------------------------------------------------------#
    ...
    def forward(self, bbox, label, padding_mask, reconst=False,ricoLabel=-1):
        B, N, _ = bbox.size()
        b = self.fc_bbox(bbox)
        ...
        if self.num_ricoLabel != -1:
            l = self.emb_label_new(label)  # by ljw
            r = self.emb_ricoLabel(ricoLabel)
            x = self.enc_fc_in(torch.cat([b, l,r], dim=-1))
